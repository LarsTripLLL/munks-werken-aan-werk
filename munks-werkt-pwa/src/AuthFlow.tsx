...
export function AuthFlow({ repository, initialMfa, onAuthenticated }: { repository: AuthRepository; initialMfa?: PendingMfaAuthentication; onAuthenticated: (user: SessionUser) => void }) {
  ...
  const verifyMfa = (event: FormEvent<HTMLFormElement>) => {
    ...
    void run(async () => {
      ...
      if (pendingResetPassword) {
        if (!repository.completePasswordResetMfa) throw new Error('Wachtwoordherstel met authenticator is niet beschikbaar.');
        await repository.completePasswordResetMfa(mfa.factorId, code, pendingResetPassword);
  ...
  const completeReset = (event: FormEvent<HTMLFormElement>) => {
    ...
    void run(async () => {
      ...
      const authentication=await repository.completePasswordReset(email,code,password);
      if(authentication){
        setPendingResetPassword(password);
        setMfa(authentication);
        setNotice('Bevestig het wachtwoordherstel met de actuele code uit je authenticator-app.');
        setScreen(authentication.status==='mfa_enroll'?'mfa-enroll':'mfa-challenge');
        return;
      }
      setNotice('Je wachtwoord is gewijzigd. Log nu opnieuw in.');
      setScreen('login');
  ...
  return <section className="auth-screen">
    ...
    {screen==='mfa-challenge'&&mfa?.status==='mfa_challenge'&&<><span className="eyebrow">Extra beveiliging</span><h1>{pendingResetPassword?'Bevestig je wachtwoordherstel':'Vul je authenticatorcode in'}</h1><p>Open je authenticator-app en vul de actuele zescijferige code voor Munks Werkt in.</p><form className="form-card" onSubmit={verifyMfa}><label>Authenticatorcode<input name="mfaCode" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} autoFocus required/></label><button className="auth-primary" disabled={busy}>{busy?'Controleren…':pendingResetPassword?'Wachtwoord veilig wijzigen':'Veilig inloggen'}</button></form><p className="auth-note">Geen toegang meer tot je authenticator? Neem contact op met je beheerder om de koppeling na identiteitscontrole te laten herstellen.</p></>}
