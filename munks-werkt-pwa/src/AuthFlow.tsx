...
export function AuthFlow({ repository, onAuthenticated }: { repository: AuthRepository; onAuthenticated: (user: SessionUser) => void }) {
  ...
  return <section className="auth-screen">
    ...
    {screen === 'privacy' && <><Progress step={2}/><span className="eyebrow">Jouw gegevens</span><h1>Jouw privacy</h1><p>We gebruiken alleen gegevens die nodig zijn voor jouw traject. Hier lees je in het kort wat dat betekent.</p><div className="form-card"><h2>Wie ziet jouw gegevens?</h2><p>Je antwoorden zijn alleen zichtbaar voor jou en de bevoegde begeleiders binnen jouw traject. De RSD ziet alleen de afgesproken voortgangs- en resultaatgegevens.</p><h2>AI-assistent en talententest</h2><p>AI kan fouten maken en neemt geen beslissingen over jou. Je begeleiders bespreken de uitslag van de talententest met je en blijven verantwoordelijk voor het uiteindelijke advies.</p><button className="document-link" type="button" onClick={() => setScreen('privacy-document')}>Open de privacyverklaring</button><label className="check"><input type="checkbox" checked={privacyAccepted} onChange={e => setPrivacyAccepted(e.target.checked)}/><span>Ik heb de privacyverklaring gelezen en ga hiermee akkoord.</span></label><button className="document-link" type="button" onClick={() => setScreen('consent-document')}>Open het toestemmingsformulier</button><label className="check"><input type="checkbox" checked={consentAccepted} onChange={e => setConsentAccepted(e.target.checked)}/><span>Ik heb het toestemmingsformulier gelezen en ga hiermee akkoord.</span></label></div><button className="auth-primary" disabled={!privacyAccepted||!consentAccepted} onClick={() => {setError('');setScreen('ai-consent')}}>Verder naar toestemming</button><button className="auth-secondary" onClick={() => setScreen('activate')}>Terug</button></>}
