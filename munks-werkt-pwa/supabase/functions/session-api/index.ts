...
Deno.serve(async request => {
  ...
  try {
    ...
    for(const row of managedUserRows){const role=row.global_role==='functional_admin'?'project_leader':row.staff_role==='rsd_user'?'commissioner':'coach';const current=managedUsersMap.get(row.id)??{id:row.id,name:row.name,email:row.email,role,organization:row.organization??(role==='commissioner'?'Opdrachtgever':'Munks Werkt'),commissionerCode:role==='commissioner'?(row.commissioner_code??undefined):undefined,trajectoryCodes:[],active:row.active};if(row.trajectory_code&&!current.trajectoryCodes.includes(row.trajectory_code))current.trajectoryCodes.push(row.trajectory_code);managedUsersMap.set(row.id,current)}
